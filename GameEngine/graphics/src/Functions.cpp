...
namespace GameBoi { namespace graphics {
	...
	void Window::resize(int width, int height)
	{
		...
		if (config->fullscreen == 1)
			glfwSetWindowMonitor(m_Window, glfwGetPrimaryMonitor(), 0,0, width , height, 60);
		else
			glfwSetWindowMonitor(m_Window, NULL, 0,0, width , height, 60);
	...
	void Window::drawHitbox(float topLeftX, float topLeftZ, float scale){
		...
		float vertices[] = {
			topLeftX,  topLeftZ, // top left
			topLeftX - scale, //top right
			topLeftX -scale, topLeftZ-scale,//bot right
			topLeftX, topLeftZ-scale //bot left
		};
